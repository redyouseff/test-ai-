...
const HealthyTalk = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<
    "Articles" | "Case Studies" | "Research" | null
  >(null);
  ...
  const likeMutation = useMutation({
    mutationFn: async ({ postId }: { postId: string }) => {
      const response = await fetch(
        getApiUrl(`/api/v1/health-talks/${postId}/like`),
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        const error = await response
          .json()
          .catch(() => ({ message: "Failed to like post" }));
  ...
  const commentMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(
        getApiUrl(`/api/v1/health-talks/${postId}/comments`),
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const filteredPosts = healthPosts.filter((post) => {
    ...
    const matchesSpecialty =
      !selectedSpecialty || post.doctor.specialty === selectedSpecialty;
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-xl text-muted-foreground text-center max-w-2xl">
            Expert medical insights, tips, and discussions from our community of
                        ...
                        {post.tags.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-3">
                            {post.tags.map((tag) => (
                              <span
                                key={tag}
                                className="bg-accent px-2 py-0.5 text-xs rounded-full"
                              >
                                {tag}
                              </span>
                            ))}
                        ...
                        <div className="flex justify-between items-center w-full mb-4">
                          <div className="flex gap-4">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                setIsCommenting(
                                  isCommenting === post.id ? null : post.id
                                )
                              }
                            >
                        ...
                        {post.comments.length > 0 && (
                          <div className="w-full space-y-3">
                            {post.comments.slice(0, 2).map((comment) => (
                              <div
                                key={comment.id}
                                className="flex gap-2 p-2 rounded-md bg-secondary/50"
                              >
